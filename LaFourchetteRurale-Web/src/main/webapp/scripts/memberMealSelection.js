...
import {getNowDate, getNextMonday, formattedDate, getNextDay, generateDateList, generateDateListAlternate, generateDateListAlternate2} from "../scripts/datesOperations.js"
...
let deliveryDate = JSON.parse(localStorage.getItem("dayOfDeliveryMember"))
...
export function findDietNameOfMemberByDietId(){
    ...
    let memberDiet
        for (let i = 0; i < dietList.length; i++) {
            if (dietList[i].dietId === member.diet) {
